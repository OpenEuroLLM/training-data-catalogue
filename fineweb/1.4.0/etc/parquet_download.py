...
def main(output_dir, crawl_list_path, max_workers, num_attempts=5, sleep_interval=10, delta_gib=100):
    ...
    for ccsplit in crawls:
        ...
        if should_download:
            for attempt in range(num_attempts):
                ...
                elif abs(downloaded_gbytes - expected_size) > delta_gib:
                    print(f"Large mismatch for {ccsplit}: expected {expected_size}G, got {downloaded_gbytes}G")
                    failed = True
